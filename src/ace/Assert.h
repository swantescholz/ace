...
#define __FFL ace::AssertionPoint{__FILE__,__func__,std::to_string(__LINE__)}
#define assertFail(X) ace::__mythrow(ace::AssertionFailException(std::string(X),__FFL));
#define assertTrue(X) if (!(X)) {ace::__mythrow(ace::AssertionTrueException(#X,__FFL));}
#define assertFalse(X) if (X) {ace::__mythrow(ace::AssertionFalseException(#X,__FFL));}
#define assertEqual(X,Y) {auto __x = (X); auto __y = (Y); if (!(__x == __y)) { \
	ace::__mythrow(ace::AssertionEqualException<decltype(__x),decltype(__y)>(__x,#X,__y,#Y,__FFL));}}
#define assertUnequal(X,Y) {auto __x = (X); auto __y = (Y); if (!(__x != __y)) { \
	ace::__mythrow(ace::AssertionUnequalException<decltype(__x),decltype(__y)>(__x,#X,__y,#Y,__FFL));}}
#define assertLessThan(X,Y) {auto __x = (X); auto __y = (Y); if (!(__x < __y)) { \
	ace::__mythrow(ace::AssertionLessThanException<decltype(__x),decltype(__y)>(__x,#X,__y,#Y,__FFL));}}
#define assertGreaterThan(X,Y) {auto __x = (X); auto __y = (Y); if (!(__x > __y)) { \
	ace::__mythrow(ace::AssertionGreaterThanException<decltype(__x),decltype(__y)>(__x,#X,__y,#Y,__FFL));}}
#define assertClose(X,Y) {auto __x = (X); auto __y = (Y); if (!ace::Util::getInstance().almostEqual(__x,__y)) { \
	ace::__mythrow(ace::AssertionCloseException(__x,#X,__y,#Y,__FFL));}}
#define assertCloserThan(X,Y,PRECISION) {auto __x = (X); auto __y = (Y); auto __precision = (PRECISION); if (!ace::Util::getInstance().almostEqual(__x,__y,__precision)) { \
	ace::__mythrow(ace::AssertionCloserThanException(__x,#X,__y,#Y,__precision,#PRECISION,__FFL));}}
